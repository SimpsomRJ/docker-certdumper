...
dump() {
  ...
  if [ "${#DOMAINS[@]}" -gt 1 ]; then
    for i in "${DOMAINS[@]}" ; do
      ...
      then
        ...
        then
          ...
          openssl pkcs12 -export -passout pass: -nokeys -in ${dir}/cert.pem \
            -inkey ${dir}/key.pem -out ${dir}/cert.pfx
        fi
      else
    ...
    then
      ...
      then
        ...
        openssl pkcs12 -export -passout pass: -nokeys -in ${outputdir}/cert.pem \
          -inkey ${outputdir}/key.pem -out ${outputdir}/cert.pfx
      fi
    else
  ...
  if [[ ! -z "${OVERRIDE_UID}" && ! -z "${OVERRIDE_GID}" ]]; then
    if [[ ! "${OVERRIDE_UID}" =~ $re || ! "${OVERRIDE_GID}" =~ $re ]]; then
      ...
      find ${outputdir}/ -type f \( -name "*.pem" -o -name "*.pfx" \) -print0 \
        | xargs chown "${OVERRIDE_UID}":"${OVERRIDE_GID}"
      find ${outputdir}/ -type f -name "*.pfx" -print0 | xargs chmod 600
    fi
  fi
